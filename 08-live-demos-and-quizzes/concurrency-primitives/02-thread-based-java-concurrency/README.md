...
## Quick: Run in two modes (platform or virtual) — no `javac`
...
From anywhere on your machine you can run the demo directly with the single-file Java launcher (no `javac` required). Use the absolute path below:

```bash
# Run platform threads (default 1000 if you omit a number)
...
# Run virtual threads (requires JDK 21+; default 1_000_000 if omitted)
...
- The single-file `java` launcher compiles and runs the source in one step (works on Java 11+).
...
java /tmp/concurrency-in-a-nutshell/08-live-demos-and-quizzes/concurrency-primitives/02-thread-based-java-concurrency/src/ThreadBasedJavaConcurrency.java platform 10
